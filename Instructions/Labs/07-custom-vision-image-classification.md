...
1. Téléchargez les images d’entraînement depuis `https://github.com/MicrosoftLearning/mslearn-ai-vision/raw/main/Labfiles/07-custom-vision-image-classification/training-images.zip` et extrayez le dossier compressé pour afficher son contenu. Ce dossier contient des sous-dossiers d’images de pommes, de bananes et d’oranges.
...
1. De retour dans le Portail Azure, exécutez la commande `cd C-Sharp/train-classifier` ou `cd Python/train-classifier` selon votre langue choisie.
...
1. En utilisant la commande `ls`, vous pouvez afficher le contenu du dossier **train-classifier**, et noter qu’il contient un fichier pour les paramètres de configuration :
    - **C#** : appsettings.json
    - **Python** : .env
...
1. Dans le cloud shell, exécutez la commande `cd ../test-classifier`, puis saisissez la commande spécifique au kit de développement logiciel (SDK) pour installer le package de prédiction Custom Vision :
...
1. En utilisant la commande `ls`, vous pouvez afficher le contenu du dossier **test-classifier**, qui sont utilisés pour implémenter une application cliente de test pour votre modèle de classification d’image.
...
1. Fermez l’éditeur de code et saisissez la commande spécifique au kit de développement logiciel (SDK) suivante pour exécuter le programme :
